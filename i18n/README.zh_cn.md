<!--
  Satoru's GitHub Profile README
  Banner recommendation: 2400x300 or 3200x360, very wide and narrow.
-->


<!-- <p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="../assets/banner-retro-anime-up.png">
    <source media="(prefers-color-scheme: light)" srcset="../assets/banner-retro-anime-up.png">
    <img
      src="../assets/banner-retro-anime-up.png"
      alt="Retro anime style long up banner for Satoru's GitHub profile"
      width="100%"
      height="100%"
    />
  </picture>
</p> -->

<h1 align="center">渡辺暁</h1>

<p align="center">
  <img src="https://github.com/AonoChano.png" width="110" height="110" alt="Satoru 的头像" style="border-radius:50%;" />
</p>

<p align="center">
  <em>机器人会梦见电子山羊吗？</em>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/渡辺-6c5ce7?style=for-the-badge" />
  <img src="https://img.shields.io/badge/暁-2f3542?style=for-the-badge" />
  <img src="https://img.shields.io/badge/华中师范大学-学生-00b894?style=for-the-badge" />
  <img src="https://img.shields.io/badge/专业-电子信息-0984e3?style=for-the-badge" />
</p>
<p align="center">
  🌐 <a href="../README.md">English</a> · <a href="./README.zh_cn.md">简体中文</a> · <a href="./README.zh_tw.md">繁體中文</a> · <a href="./README.ja.md">日本語</a>
</p>

<br></br>

## 关于我

你好！我是**渡辺暁**（わたなべ あきら / Satoru Watanabe）。

我是华中师范大学电子信息科学与技术专业的本科生。

我热衷于构建事物、理解它们的工作原理，并通过实践来学习。目前，我的大部分时间都花在探索 AI、大语言模型、软件工程和嵌入式系统上。

我喜欢把有趣的想法变成真正的项目。无论是一个小型开发者工具、一个嵌入式实验，还是一个 AI 工作流，我不仅想了解它*如何*工作，还想了解它*为什么*这样设计。

---

## 我在做什么

这些是我目前花费大部分时间的领域：

- 🤖 AI 智能体和大语言模型应用
- 🛠 开发者工具和自动化
- 💻 软件工程和开源项目
- 🔌 嵌入式系统和电子技术
- 📚 夯实机器学习和自动控制的基础

我喜欢尝试新想法、阅读源代码，并构建能够解决日常开发中实际问题的小项目。

## Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=c,cpp,python,git,github,linux,bash,powershell,ts,js,html,react,rust,go,arduino" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/AI Engineering-222831?style=flat-square" />
  <img src="https://img.shields.io/badge/Embedded%20Systems-393E46?style=flat-square" />
  <img src="https://img.shields.io/badge/Open%20Source-00ADB5?style=flat-square" />
  <img src="https://img.shields.io/badge/Developer%20Tools-6C5CE7?style=flat-square" />
</p>

---

## 当前正在探索

- 学习机器学习基础知识
- 研究自动控制理论
- 构建 AI 辅助开发工作流
- 实验多智能体系统
- 开发让日常工作更愉快的工具
- 阅读有趣的开源项目源代码

---

## 精选项目

| 项目 | 描述 |
| :--- | :--- |
| [Telegodex](https://github.com/AonoChano/telegodex) | 从 Telegram 远程运行和管理 Codex、Claude Code 等编程智能体。 |
| [KORT](https://github.com/AonoChano/knights-of-round-table) | 一个实验性框架，多个 AI 专家在此协作、辩论并共同解决问题。 |
| [ClaudeSwitchTUI](https://github.com/AonoChano/claude-switch-tui) | 安全切换 Claude Code 提供商的多语言 TUI 工具，支持操作系统密钥链集成和隔离启动环境。 |

<!-- 
---

<p align="center">
  <picture>
    <source
      media="(prefers-color-scheme: dark)"
      srcset="https://raw.githubusercontent.com/AonoChano/AonoChano/output/github-contribution-grid-snake-dark.svg"
    />
    <source
      media="(prefers-color-scheme: light)"
      srcset="https://raw.githubusercontent.com/AonoChano/AonoChano/output/github-contribution-grid-snake.svg"
    />
    <img
      alt="github contribution grid snake animation"
      src="https://raw.githubusercontent.com/AonoChano/AonoChano/output/github-contribution-grid-snake.svg"
    />
  </picture>
</p> -->


---


## 除了编程之外

除了编程，我还喜欢动漫、科幻小说，以及探索新技术如何改变人们构建软件的方式。

我很高兴与大家交流关于 AI、开源或任何有趣事物的想法。

---

## 联系方式

<p align="center">
  <a href="https://github.com/AonoChano">
    <img src="https://img.shields.io/badge/GitHub-AonoChano-181717?style=for-the-badge&logo=github" />
  </a>
  <a href="mailto:AonoChano@outlook.com">
    <img src="https://img.shields.io/badge/Email-联系我-D14836?style=for-the-badge&logo=outlook&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <em>“前天我看见了一只兔子，昨天看见了一只鹿，而今天，我看见了你。”</em>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=AonoChano&style=flat-square&color=6c5ce7" alt="Profile views" />
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="../assets/banner-retro-anime-down.png">
    <source media="(prefers-color-scheme: light)" srcset="../assets/banner-retro-anime-down.png">
    <img
      src="../assets/banner-retro-anime-down.png"
      alt="Retro anime style long down banner for Satoru's GitHub profile"
      width="100%"
      height="100%"
    />
  </picture>
</p>